...
│
├── notebooks/
│   └── cross_validation_report.ipynb  # Executed CV study: code + narrative in one place
│
├── report/
│   └── cross_validation_report.docx   # Formal report: tables, strengths/weaknesses, verdict
│
├── src/
...
│
├── scripts/
│   ├── build_notebook.py           # Rebuilds + executes the CV notebook
│   └── build_docx.py               # Rebuilds the docx report from results/cv_results.csv
│
├── results/
...
Generated by `src/plots.py`.

## 📓 Cross-validation report

- **`notebooks/cross_validation_report.ipynb`** — executed notebook conducting the full CV study (data analysis → feature engineering → 30-job cluster CV → all result tables → paired model comparison), with narrative notation throughout.
- **`report/cross_validation_report.docx`** — formal report with the same tables and numbers, model strengths/weaknesses, and the overall verdict (recommended model: LogReg, all features, 0.343 screening threshold).

Rebuild either with `py scripts/build_notebook.py` / `py scripts/build_docx.py`.
