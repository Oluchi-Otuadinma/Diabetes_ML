...
- **`notebooks/cross_validation_report.ipynb`** — executed notebook: the full report of everything done, conducted live end-to-end (data analysis → feature engineering → 30-job cluster CV → default & high-recall tables → all 7 project figures → paired model comparison → strengths/weaknesses → verdict), with narrative notation throughout.
- **`report/cross_validation_report.docx`** — formal report with the same tables and numbers, all 7 figures embedded, model strengths/weaknesses, and the overall verdict (recommended model: LogReg, all features, 0.343 screening threshold).
...
```bash
git clone https://github.com/Oluchi-Otuadinma/Diabetes_Dask.git
cd Diabetes_Dask
